    ...
    <div class="p-2">
      <div v-for="message in messages" :key="message" class="flex flex-col gap-2 mb-4">
        ...
        <TransitionGroup name="item" tag="button" class="self-end flex flex-col gap-2">
          <button v-for="response in message.responses_select" :key="response.response_id" :disabled="message.message.message_id != current" @click="chat(response.response_id); message.responses_select = [response]; response.selected = true" class="self-end">
